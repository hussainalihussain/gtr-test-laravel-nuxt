...
await getToken();
await cartLoader();
const processRemoveFromCart = async (cart_id) => {
    await cart.removeFromCart(cart_id)
        ...
        <div class="flex shadow-md my-10">
            <div class="w-3/4 bg-white px-10 py-10">
                <div class="flex justify-between border-b pb-8">
                    ...
                    <h2 class="font-semibold text-2xl">{{ cart.totalItems }} {{ cart.totalItemsPhrase }}</h2>
                </div>
                ...
                <div
                    ...
                    <div
                        class="flex w-2/5">
                        <div class="w-20">
                            <img class="h-24" :src="item.product?.images[0].src" alt="">
                        </div>
                        <div class="flex flex-col justify-between ml-4 flex-grow">
                            <span class="font-bold text-sm">{{ item.product.title }}</span>
                            <span class="text-red-500 text-xs">{{ item.product.category }}</span>
                            <button @click="processRemoveFromCart(item.id)" class="font-semibold hover:text-red-500 text-gray-500 text-xs">Remove</button>
                        </div>
                    </div>
                ...
                <NuxtLink href="/" class="flex font-semibold text-indigo-600 text-sm mt-10">
            ...
            <div id="summary" class="w-1/4 px-8 py-10">
                <h1 class="font-semibold text-2xl border-b pb-8">Order Summary - dummy</h1>
